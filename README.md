...
Every layer — from a signed order over a real WebSocket relay, through an LP that provides liquidity, to on-chain settlement — is built, tested, and **proven live on real pearld nodes** (49 unit/integration tests + a live on-chain swap test, typecheck clean).
...
- **Next:** a real BTC↔PRL run once the signet wallet is funded (the `BitcoinClient` is built + unit-tested; only the faucet blocks it); the maker commitment bond (closes the documented forfeit-griefing gap); and the wallet/venue UX (the real moat). The `OP_CAT` covenant remains an optional later PoC, not a dependency. See `DESIGN.md` §9.
