...
- **Done:** ported atomic-swap primitives (`SwapTree`, `Timelocks`, `Funder`, `ChainClient`); the secret-tied `Bond`; the `SwapPlan` two-user layout; the operator `Fee` + signed `OrderIntent`s; the **P2P coordinator** — a crash-safe, idempotent, fee-bumped (RBF) state machine (`SwapExecutor` + `SwapStore`); the **matching/relay layer** — crossing matcher (`OrderBook`, price-time priority, partial fills, fee/expiry/signature gates), settlement handshake (`Handshake`), transport-abstracted relay (`RelayServer`), flat-file discovery registry (`FileRegistry`); the **per-user client** (`SwapClient`) tying relay + handshake + executor together with crash-recovery (`resume()`); a reference **`SwapWallet`** (`ReferenceWallet`) that funds lockups with real key-path taproot transactions; the **live chain clients** (`PearlClient` over pearld btcd-RPC, `BitcoinClient` over bitcoind Core-RPC, sharing an incremental block-scanning `RpcChainClient`); a **WebSocket relay** (`WsRelayServer` + `connectWsRelay`) so separate parties connect to one operator over a real socket; and a **third-party LP daemon** (`MarketMaker` + `SpreadPolicy`) — an always-on, inventory-sized market maker built on `SwapClient` that keeps fresh resting quotes on the book so a lone taker always finds a counterparty (the cold-start fix), with the operator never providing liquidity. E2Es drive the **whole pipe**: two `SwapClient`s post crossing signed orders → relay matches → handshake *through* the relay → both derive byte-identical terms → executors settle on-chain (maker learns the preimage from the chain); a wallet E2E settles a full two-party swap with real funding txs; the chain clients are tested against an in-memory JSON-RPC stub (Core `tx` vs btcd `rawtx` shapes, confirmation maturation); plus coordinator E2Es (taker walk → maker forfeit, counterparty-no-fund → refund + bond reclaim, restart/resume, RBF bump) and matcher/handshake/registry/wallet unit tests.
- **Maker-grief gap (documented, mitigated):** we proved a symmetric "maker commitment bond" can't be made sound with hashlocks — the two stall cases are on-chain-indistinguishable (see [`docs/maker-grief-analysis.md`](./docs/maker-grief-analysis.md)). v1 ships the taker option bond + a relay **reputation** gate (`MakerReputation`) that refuses repeat griefers; the cryptographic close is the optional OP_CAT covenant. The residual: a first-time grief can cost a taker its ~1–2% bond (principal always safe).
- **Next:** a real BTC↔PRL run once the signet wallet is funded (the `BitcoinClient` is built + unit-tested; only the faucet blocks it); and the wallet/venue UX (the real moat). The `OP_CAT` covenant remains an optional later PoC, not a dependency. See `DESIGN.md` §9.
